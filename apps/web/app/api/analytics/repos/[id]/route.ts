...
export async function GET(
  ...
  try {
    ...
    // Fetch check runs for this repo
    const { data: runs, error: runsError } = await supabase
      .from("check_runs")
      .select("id, status, created_at, is_agent_commit, commit_author, total_checks, passed_checks, failed_checks, warned_checks")
      .eq("repo_id", repoId)
    ...
    for (const run of runs || []) {
      ...
      const entry = dailyMap.get(day) || { total: 0, passed: 0, failed: 0 };
      entry.total++;
    ...
    // Failure breakdown by check_type
    ...
    const failureBreakdown = Array.from(failureMap.entries())
    ...
    const agentVsHuman = {
      agent: {
        ...
        passed: agentRuns.filter((r) => r.status === "passed").length,
        passRate: agentRuns.length > 0
          ? Math.round((agentRuns.filter((r) => r.status === "passed").length / agentRuns.length) * 1000) / 10
          : 0,
      },
      human: {
        total: humanRuns.length,
        passed: humanRuns.filter((r) => r.status === "passed").length,
        passRate: humanRuns.length > 0
    ...
    return NextResponse.json({
      repoId,
      range,
      summary: { totalRuns, passedRuns, passRate },
      dailyPassRate,
      failureBreakdown,
      agentVsHuman,
    });
